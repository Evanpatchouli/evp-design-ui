...
export default function EvpFlexbarLabelfixed(props: EvpFlexbarProps) {
  ...
  const $radius = props.labelRadius
    ? typeof props.labelRadius === "number"
      ? `${props.labelRadius}px`
      : props.labelRadius
    : `calc(${height} * 0.5)`;
  ...
  const borderStyle = `${props.border?.width ?? "1px"} ${
    props.border?.color ?? Color.PaleGray
  } ${props.border?.lineStyle ?? "solid"}`;
  ...
  return (
    ...
    >
      <div
        className={`evp-flexbar ${props.class ?? ""}`.trim()}
        onMouseLeave={() => {
        ...
        style={{
          ...
          left:
            labelPosition === "left"
              ? (expand ? -1 : 0) * (expandDistand as number)
              : void 0,
          top:
            labelPosition === "top"
              ? (expand ? -1 : 0) * (expandDistand as number)
      ...
      >
        <div
          className={`evp-flexbar-content-wrapper ${
            props.contentClass ?? ""
          }`.trim()}
          style={{
            height: ["right", "left"].includes(labelPosition)
              ? height
              : expand
              ? ref.current?.clientHeight
              : 0,
            width: ["top", "bottom"].includes(labelPosition)
              ? height
              : expand
              ? ref.current?.clientWidth
              : 0,
            writingMode: ["right", "left"].includes(labelPosition)
              ? "horizontal-tb"
              : "vertical-lr",
            border: `1px ${Color.PaleGray} solid`,
            borderTop: ["right", "left"].includes(labelPosition)
              ? borderStyle
              ...
              ? expand
                ...
                : "none"
              : "none",
            borderBottom: ["right", "left"].includes(labelPosition)
              ? borderStyle
              : labelPosition === "top"
              ? expand
                ? borderStyle
                : "none"
              : "none",
            borderLeft: ["top", "bottom"].includes(labelPosition)
              ? borderStyle
              : labelPosition === "right"
              ? expand
                ? borderStyle
                : "none"
              : "none",
            borderRight: ["top", "bottom"].includes(labelPosition)
              ? borderStyle
              : labelPosition === "left"
              ? expand
                ? borderStyle
        ...
        <div
          className={`evp-flexbar-label ${props.labelClass ?? ""}`.trim()}
          onClick={() => {
