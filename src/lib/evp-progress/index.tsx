...
export const EvpProgress: React.FC<EvpProgressProps> = ({ value, onChange, ...props }) => {
  ...
  return (
    <div
      className={classNames(styles["evp-progress"], props.class)}
      id={props.id}
      style={{
        // @ts-ignore
        "--size":
      ...
      {...pickEvents(props)}
    >
      <div className={styles["evp-progress__extra"]}></div>
      <div className={styles["evp-progress__fill"]}></div>
      <div className={styles["evp-progress__mask"]}>
        <span>{`${rate()}%`}</span>
      </div>
    </div>
  );
