...
export default function EvpPopover(props: EvpPopoverProps) {
  ...
  const left = () => {
    if (["topLeft", "bottomLeft"].includes(props.position ?? "top")) {
      ...
      ["left", "leftTop", "leftBottom", "upperLeftCorner", "lowerLeftCorner"].includes(props.position ?? "top")
    ) {
      return 0;
    } else if (["topRight", "bottomRight"].includes(props.position ?? "top")) {
      return (childRef.current?.clientWidth ?? 0) * 0.8;
    } else if (
      ["right", "rightTop", "rightBottom", "upperRightCorner", "lowerRightCorner"].includes(props.position ?? "top")
    ) {
  ...
  const top = () => {
    if (["top", "topLeft", "topRight", "upperLeftCorner", "upperRightCorner"].includes(props.position ?? "top")) {
      ...
      ["bottom", "bottomLeft", "bottomRight", "lowerLeftCorner", "lowerRightCorner"].includes(props.position ?? "top")
    ) {
  ...
  return (
    <div className={`evp evp-popover-container`}>
      <div
        className={`evp-popover ${props.class ?? ""} ${show ? "" : "evp-popover__hidden"}`.trim()}
        id={props.id}
      ...
      >
        ...
        <svg className="evp-popover-arrow" width={16} height={10} viewBox="0 0 16 10">
          <polygon points="0,0 16,0 8,10" fill="#ffffff" />
        </svg>
      </div>
      <div
